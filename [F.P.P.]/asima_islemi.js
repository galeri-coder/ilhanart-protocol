...
console.log("🚛 NAKLİYE BAŞLADI: [PoArt] HEDEFİNE TAŞINIYORUZ...");

// HEDEF: [PoArt]
const targetRoot = path.join(__dirname, '[PoArt]');

// YANLIŞ KLASÖRLERİ TEMİZLE (Varsa siler)
const wrongFolders = ['FPP', '[F.P.P]', 'fpp', 'F.P.P'];
wrongFolders.forEach(folder => {
    const wrongPath = path.join(__dirname, folder);
    if (fs.existsSync(wrongPath)) {
        console.log(`🧹 Temizlik: Yanlış klasör siliniyor (${folder})...`);
        fs.rmSync(wrongPath, { recursive: true, force: true });
    }
});

// HEDEF KLASÖRÜ SIFIRLA VE HAZIRLA
...
console.log("✅ [PoArt] Klasörü Hazırlandı.");

// =======================================================
// 1. CSS (PREMIUM ŞEFFAF TEMA)
// =======================================================
...
// =======================================================
// 2. SANATÇI DOSYALARI (MODULES)
// =======================================================

// 🏛️ MICHELANGELO (Leaderboard)
...
<head>
    ...
    <title>İlhanArt | [PoArt] Registry</title>
...
console.log("🚛 TAŞIMA TAMAMLANDI! [PoArt] İÇİ DOLDU, ESKİLER SİLİNDİ.");